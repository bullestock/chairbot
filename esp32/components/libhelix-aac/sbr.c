...
int InitSBR(AACDecInfo *aacDecInfo)
{
	...
	psi = (PSInfoSBR *)helix_malloc(sizeof(PSInfoSBR));
	if (!psi) {
...
int InitSBRPre(AACDecInfo *aacDecInfo, void **ptr, int *sz)
{
	...
	*ptr = (void*)((char*)(*ptr) + sizeof(PSInfoSBR));
        aacDecInfo->psInfoSBR = psi;
        aacDecInfo->sbrOwnsMemory = 0;  /* preallocated — do not free */
        return ERR_AAC_NONE;
}
...
{
	if (aacDecInfo && aacDecInfo->psInfoSBR && aacDecInfo->sbrOwnsMemory)
		helix_free(aacDecInfo->psInfoSBR);
