...
/* __attribute__ ((section (".data"))) */ static void DecodeSectionData(BitStreamInfo *bsi, int winSequence, int numWinGrp, int maxSFB, unsigned char *sfbCodeBook)
{
	...
	for (g = 0; g < numWinGrp; g++) {
		sfb = 0;
		while (sfb < maxSFB) {
			/* Guard: if bitstream is exhausted, stop parsing to avoid
			 * spinning forever when corrupt/truncated data yields sectLen == 0.
			 * Zero-fill remaining sfbCodeBook entries so downstream functions
			 * (DecodeScaleFactors, etc.) read a safe codebook value (0). */
			if (bsi->nBytes == 0 && bsi->cachedBits <= 0) {
				int remaining = maxSFB - sfb;
				while (remaining-- > 0)
					*sfbCodeBook++ = 0;
				for (g++; g < numWinGrp; g++) {
					remaining = maxSFB;
			...
			/* Guard: sectLen must advance sfb; if zero (corrupt data) force advance */
			if (sectLen == 0)
				sectLen = 1;
