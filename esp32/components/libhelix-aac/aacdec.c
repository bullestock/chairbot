...
HAACDecoder AACInitDecoder(void)
{
	...
	if (InitSBR(aacDecInfo)) {
		/* SBR init failed (OOM) — continue without SBR.
		 * The decoder will still work for base AAC-LC content.
		 * HE-AAC streams will decode at the base sample rate (e.g. 22050 Hz
		 * instead of 44100 Hz) but will still produce audio rather than failing. */
		aacDecInfo->psInfoSBR = 0;  /* NULL — no SBR processing */
	}
#endif
...
void AACGetLastFrameInfo(HAACDecoder hAACDecoder, AACFrameInfo *aacFrameInfo)
{
	...
	if (!aacDecInfo) {
		...
		/* SBR doubles sample rate and output samples, but only if psInfoSBR was
		 * successfully allocated.  When SBR init failed (OOM), sbrEnabled may be
		 * set by the bitstream parser but no SBR processing actually occurs. */
		int sbrActive = aacDecInfo->sbrEnabled && aacDecInfo->psInfoSBR;
		...
		aacFrameInfo->sampRateOut =   aacDecInfo->sampRate * (sbrActive ? 2 : 1);
		aacFrameInfo->bitsPerSample = 16;
		aacFrameInfo->outputSamps =   aacDecInfo->nChans * AAC_MAX_NSAMPS * (sbrActive ? 2 : 1);
...
#ifdef AAC_ENABLE_SBR
		if (aacDecInfo->sbrEnabled && aacDecInfo->psInfoSBR && (aacDecInfo->currBlockID == AAC_ID_FIL || aacDecInfo->currBlockID == AAC_ID_LFE)) {
