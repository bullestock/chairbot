...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    const uint8_t *hdr;
    bs_t bs_frame[1];
    /* Static to avoid ~16KB stack allocation that overflows FreeRTOS task stacks
     * on ESP32 (typical decode task stack is 12KB).  Thread-safe because only one
     * decode task (MP3 or AAC, never both) calls this function at a time. */
