...
bool MP3Decoder::decodeFile(const std::string& filePath, int16_t** pcmBuffer, size_t* pcmSize, MP3Info* info) {
    if (!_initialized) {
        return false;
    }
    ...
    size_t bytesRead = fread(mp3Data, 1, fileSize, file);
    fclose(file);
...
bool MP3Decoder::getFileInfo(const std::string& filePath, MP3Info* info) {
    ...
    size_t readSize = std::min(fileSize, (size_t)4096);
    ...
    ESP_LOGI(TAG, "MP3 Info: SampleRate=%d Hz, Channels=%d, BitRate=%d kbps\n", info->sampleRate, info->channels, info->bitRate);
...
bool MP3Decoder::startStreaming(const std::string& filePath, StreamCallback callback) {
    if (!_initialized || _streaming) {
        return false;
    }
    
    // Open the file
    _streamFile = fopen(filePath.c_str(), "r");
    if (!_streamFile) {
        return false;
    }
...
bool MP3Decoder::processStreamFrame() {
    ...
    if (_bytesLeft < INPUT_BUFFER_SIZE && !feof(_streamFile)) {
        fillStreamBuffer();
    }
...
bool MP3Decoder::fillStreamBuffer() {
    ...
    if (!_streamFile || feof(_streamFile)) {
        return false;
    }
    ...
    size_t bytesRead = fread(_streamBuffer + _bytesLeft, 1, spaceLeft, _streamFile);
