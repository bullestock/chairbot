...
static void tusb_device_task(void *arg)
{
    while (1) {
        tud_task();
        vTaskDelay(1);  /* Yield to prevent CPU burn on single-core ESP32-S2 */
    }
}
...
esp_err_t uac_device_init(uac_device_config_t *config)
{
    ...
    if (ret_val != pdPASS) {
        ESP_LOGE(TAG, "Failed to create usb_mic task");
        free(s_uac_device);
        s_uac_device = NULL;
        return ESP_FAIL;
    }
    ...
    if (ret_val != pdPASS) {
        ESP_LOGE(TAG, "Failed to create usb_spk task");
        free(s_uac_device);
        s_uac_device = NULL;
        return ESP_FAIL;
    }
